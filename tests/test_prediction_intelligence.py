...
class PredictionIntelligenceTests(unittest.TestCase):
    ...
    def test_lower_confidence_produces_wider_interval(self) -> None:
        ...
        with TemporaryDirectory() as tmp, patch(
            "shared.vm_core.prediction_intelligence.risk_adjusted_canonical_opportunities", return_value=rows,
        ), patch(
            "shared.vm_core.prediction_intelligence.canonical_review_calibration_summary",
        ...
        self.assertLess(high["upper_bound"] - high["lower_bound"], low["upper_bound"] - low["lower_bound"])
    ...
    def test_risk_adjusted_score_drives_forecast_not_raw_opportunity_score(self) -> None:
        ...
        with TemporaryDirectory() as tmp, patch(
            "shared.vm_core.prediction_intelligence.risk_adjusted_canonical_opportunities", return_value=[low, high],
        ), patch(
            "shared.vm_core.prediction_intelligence.canonical_review_calibration_summary",
            return_value={"status": "INSUFFICIENT_DATA", "known_binary_outcomes": 0, "positive_rate": None},
        ):
    ...
    def test_bad_limit_and_no_evidence_fail_safely(self) -> None:
        with TemporaryDirectory() as tmp, patch(
            "shared.vm_core.prediction_intelligence.risk_adjusted_canonical_opportunities", return_value=[],
        ), patch(
            "shared.vm_core.prediction_intelligence.canonical_review_calibration_summary",
            return_value={"status": "INSUFFICIENT_DATA", "known_binary_outcomes": "bad", "positive_rate": None},
        ):
            summary = prediction_summary(root=Path(tmp), limit="bad")  # type: ignore[arg-type]
