...
$PreSnapshot=$env:VM_PRE143_SNAPSHOT
...
function Restore-Pre143 {
    param([string]$Snapshot)
    if(-not $Snapshot -or -not (Test-Path -LiteralPath $Snapshot -PathType Container)){ return }
    Write-Host "[ROLLBACK] Restoring pre-v1.4.3 maintenance state..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host " VM PLATFORM v1.4.3 - SELF-DIAGNOSING MAINTENANCE" -ForegroundColor Cyan
    Write-Host "================================================================" -ForegroundColor Cyan
    ...
    Write-Host "[3/10] Previewing version-adaptive Smart Auto Poster maintenance..." -ForegroundColor Cyan
    py .\vm.py sap-repair
    if($LASTEXITCODE -ne 0){throw 'No safe local Smart Auto Poster repair path was found.'}

    Write-Host "[4/10] Validating/preserving current Smart Auto Poster files behind its current test suite..." -ForegroundColor Cyan
    ...
    Write-Host "[9/10] Restarting managed services on v1.4.3..." -ForegroundColor Cyan
    ...
    Write-Host " VM PLATFORM v1.4.3 MAINTENANCE COMPLETE" -ForegroundColor Green
    Write-Host "================================================================" -ForegroundColor Green
    ...
    try {
        ...
        $failurePath=Join-Path $failureDir 'v143_maintenance_failure.txt'
        @(
            'VM v1.4.3 MAINTENANCE FAILURE',
    ...
    Restore-Pre143 -Snapshot $PreSnapshot
    Write-Host "[ROLLBACK] Pre-v1.4.3 platform state restored." -ForegroundColor Yellow
    if($TranscriptStarted){ try { Stop-Transcript | Out-Null } catch {} }
    exit 2
}
