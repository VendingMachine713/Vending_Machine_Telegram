...
if(-not $BackupPath){
  $BackupPath=Get-ChildItem (Join-Path $Root 'backups') -Directory -Filter 'pre_vm_intelligence_v5_*' -ErrorAction SilentlyContinue |
...
Write-Host 'VM Intelligence v5 rollback' -ForegroundColor Yellow
Write-Host "Root:   $Root"
Write-Host "Backup: $BackupPath"

# Keep a temporary copy of the bridge before tools are restored, because the pre-v5
...
foreach($rel in @(
 ...
 'docs\VM_INTELLIGENCE_v3_RELEASE_NOTES.md','docs\VM_INTELLIGENCE_v5_RELEASE_NOTES.md',
...
# repair pre-existing platform drift and were validated before v5 feature mutation.
Set-Location $Root
...
Write-Host '[OK] VM Intelligence v5 rollback completed.' -ForegroundColor Green
