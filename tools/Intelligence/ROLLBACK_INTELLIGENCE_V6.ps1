...
if(-not $BackupPath){
  $BackupPath=Get-ChildItem (Join-Path $Root 'backups') -Directory -Filter 'pre_vm_intelligence_v6_*' -ErrorAction SilentlyContinue |
    Sort-Object LastWriteTime -Descending | Select-Object -First 1 | ForEach-Object {$_.FullName}
}
if(-not $BackupPath -or -not (Test-Path $BackupPath)){throw 'No VM Intelligence v5 backup folder was found.'}
$BackupPath=(Resolve-Path $BackupPath).Path
Write-Host 'VM Intelligence v6 rollback' -ForegroundColor Yellow
Write-Host "Root:   $Root"
Write-Host "Backup: $BackupPath"

# Keep a temporary copy of the bridge before tools are restored, because the pre-v6
...
foreach($rel in @(
 ...
 'docs\VM_INTELLIGENCE_v3_RELEASE_NOTES.md','docs\VM_INTELLIGENCE_v5_RELEASE_NOTES.md','docs\VM_INTELLIGENCE_v6_RELEASE_NOTES.md',
...
# repair pre-existing platform drift and were validated before v6 feature mutation.
Set-Location $Root
...
Write-Host '[OK] VM Intelligence v6 rollback completed.' -ForegroundColor Green
