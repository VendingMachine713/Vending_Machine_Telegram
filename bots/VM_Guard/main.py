...
async def deny(update):
    # Do not create noisy permission-oracle replies inside shared groups.
...
def main():
    ...
    if not admin_id(): print(f"[CLAIM CODE] Send /claim {claim_code()} to this bot in a PRIVATE chat from your Telegram account.")
    print("[READY] VM Guard v1.1 — passive in groups, private-owner control only")
    publisher.started(mode="active" if config().get("mutations_enabled") else "monitor_only",control_scope="private_owner_only")
    try:
