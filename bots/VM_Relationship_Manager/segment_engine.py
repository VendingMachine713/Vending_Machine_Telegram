...
class SegmentEngine:
    ...
    def compute(self, telegram_id: int):
        ...
        x = self.db.one("SELECT * FROM contact_classifications WHERE telegram_id=?", (telegram_id,))
        a = self.db.one("SELECT COALESCE(MAX(action_score),0) max_score FROM recommended_actions WHERE telegram_id=? AND status IN ('open','snoozed')", (telegram_id,))
        ...
        if x and x['decision_state']=='suggested' and int(x['confidence'] or 0)>=60:
            add('classification_review', int(x['confidence']), f"Predicted {x['predicted_type']} at {x['confidence']}%")
        if a and int(a['max_score'] or 0)>=50:
            add('exception_action', int(a['max_score']), f"Recommended action score {a['max_score']}/100")
