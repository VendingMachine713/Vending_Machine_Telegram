...
KEYS = set(REQUIRED) | {"TELEGRAM_PHONE", "SESSION_NAME"}
...
print("=" * 68)
print(" VM RELATIONSHIP MANAGER - DEEP CREDENTIAL RECOVERY R2")
print("=" * 68)
...
# 5) Relationship Manager ZIPs — inspect only env-like entries.
zip_roots = [downloads]
if project_root.name == "Vending_Machine_Telegram":
...
def validate_bot_token(token: str) -> bool:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "VM-RM-Recovery/6.0.0"})
...
if phone:
    ...
    print("[!] Backup-account phone is ambiguous; it will be omitted because the saved authorised session can start without it.")
...
if phone:
    recovered["TELEGRAM_PHONE"] = phone

missing = [k for k in REQUIRED if not recovered.get(k)]
if missing:
    print()
    print("[X] Deep recovery could not safely recover:", ", ".join(missing))
    print("[X] Active .env was NOT rewritten.")
    print("[!] Required identity recovery is incomplete; no changes were written.")
    raise SystemExit(20)
...
print("[+] Deep recovery R2 complete.")
