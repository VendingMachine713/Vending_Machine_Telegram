        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                if self._due(self.db.meta("last_intelligence_maintenance"), 21600):  # 6h
                    ...
                    exception_policy = self.engine.exception_policy.summary()
                    ...
                    self._health(
                        ...
                        f"classifications={classifications['computed']}/{classifications['newly_applied']}new-auto; calibration={len(calibration)}types; actions={actions['active_signals']}; selected_exceptions={exception_policy['selected']}; "
                ...
                if self._due(self.db.meta("last_safe_self_heal"), 21600):
                    result = self.maintenance.run_safe(allow_backup=True)
                    self.db.set_meta("last_safe_self_heal", utcnow())
                    self._health("safe_self_heal", "ok" if result["status"] == "pass" else "warn",
                                 f"status={result['status']}; actions={len(result['actions'])}; remaining={len(result['after'])}")
                ...
                if self._due(self.db.meta("last_daily_backup"), 86400):
                    ...
                    self._health("daily_backup", "ok" if result["status"] == "verified" else "error",
                                 f"{result['status']} Â· {result['bytes']} bytes Â· {result['path']}")
                ...
                self.db.execute("DELETE FROM action_feedback WHERE id NOT IN (SELECT id FROM action_feedback ORDER BY id DESC LIMIT 5000)")
                self.db.execute("DELETE FROM operations_snapshots WHERE id NOT IN (SELECT id FROM operations_snapshots ORDER BY id DESC LIMIT 1000)")
                self.db.execute("DELETE FROM classification_feedback WHERE id NOT IN (SELECT id FROM classification_feedback ORDER BY id DESC LIMIT 5000)")
                ...
                self._health("operations_slo", "ok" if ops["status"] == "pass" else ops["status"], f"health={ops['health_score']}; status={ops['status']}")
