...
class AutomationEngine:
    ...
    def evaluate_contact(self,tid:int):
        ...
        if old_h and old_h != health_band:
            if horder[health_band] < horder.get(old_h,3) and score>=45:
                ...
                self._attention(tid,'red' if health_band=='critical' else 'orange','health_transition','Relationship health declined',f'Health moved {old_h} → {health_band} ({health}/100).')
    ...
    def process_opportunity_due(self):
        ...
        for o in rows:
            ...
            if int(o['health_score'] or 100)<55:
                ...
                self._attention(o['telegram_id'],'yellow' if int(o['health_score'])>=35 else 'orange','opportunity_health','Opportunity is stagnating',f"#{o['id']} {o['title']} health {o['health_score']}/100 · stale {o['stale_days']}d")
    ...
    def process_goal_due(self):
        ...
        for g in rows:
            ...
            self._attention(g['telegram_id'],priority,'goal_due','Relationship goal due',f"#{g['id']} {g['title']} · {g['progress_pct']}% · {g['next_step'] or 'Next step not set'}")
