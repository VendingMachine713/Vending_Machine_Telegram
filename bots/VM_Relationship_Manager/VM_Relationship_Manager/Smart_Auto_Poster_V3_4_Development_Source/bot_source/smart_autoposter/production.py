...
def production_readiness(db: Database, campaign_id: str, *, expected_collection: str | None = None) -> dict:
    ...
    with db.connect() as con:
        ...
        active_queue = con.execute(
            "SELECT COUNT(*) FROM queue WHERE campaign_id=? AND status IN ('pending','retry','sending','deferred')",
            (campaign_id,),
        ).fetchone()[0]
    ...
    for row in variants:
        ...
        if not row["caption"]:
            problems.append(f"empty caption: {row['content_id']}")
    ...
    if media_delivery["text_destinations_receive_caption_only"]:
        warnings.append(
            f"{text_destinations} selected text-mode destination(s) will receive caption-only posts; "
            f"album media is sent only to the {photo_destinations} photo-mode destination(s)"
        )
    ...
    *,
    ...
    with db.connect() as con:
        active_queue = int(con.execute(
            "SELECT COUNT(*) FROM queue WHERE campaign_id=? AND status IN ('pending','retry','deferred','sending')",
            (campaign_id,),
        ).fetchone()[0])
    if active_queue:
