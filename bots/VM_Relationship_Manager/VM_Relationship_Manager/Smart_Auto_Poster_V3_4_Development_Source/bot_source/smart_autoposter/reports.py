...
def report_text(db: Database, hours: int = 24, *, title: str | None = None, recommendation_limit: int = 5) -> str:
    ...
    lines=[title or f'SMART AUTO POSTER V{__version__} — {hours}H REPORT','']
    lines += [
...
def daily_report_text(db: Database) -> str:
    return report_text(db,24,title=f'SMART AUTO POSTER V{__version__} — DAILY')


def weekly_report_text(db: Database) -> str:
    return report_text(db,168,title=f'SMART AUTO POSTER V{__version__} — WEEKLY',recommendation_limit=10)
