...
class Worker:
        ...
        try:
            ...
            try:
                ids = await asyncio.wait_for(
                    self.pool.send(account, job["group_id"], job["caption"], media, job["mode"], job["topic_id"]),
    ...
    def finish_error(self, job, error, permanent=False, retry_at=None, account=None, kind: str | None = None):
        ...
        with self.db.connect() as con:
            ...
            penalize_destination = kind not in {"network", "worker_busy", "uncertain_telegram_ack", "send_timeout_uncertain", "flood_wait", "slow_mode", "no_authorized_account", "account_disabled", "account_cooldown", "account_cooldown_or_pacing"}
            ...
            if account:
                ...
                if kind not in {"slow_mode", "worker_busy", "uncertain_telegram_ack", "send_timeout_uncertain"}:
        ...
        event_type = "uncertain_send" if kind in {"uncertain_telegram_ack", "send_timeout_uncertain"} else "send_failure"
