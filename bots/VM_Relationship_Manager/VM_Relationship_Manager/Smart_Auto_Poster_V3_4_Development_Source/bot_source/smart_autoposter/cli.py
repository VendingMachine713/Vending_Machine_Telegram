...
def _json_text(value, *, machine_safe: bool = False, indent: int = 2):
    ...
    containing symbols such as ♧ cannot crash Python with UnicodeEncodeError.
    """
    return json.dumps(
        value,
        indent=indent,
...
def cmd_go_live_readiness(args):
    ...
    if int(delivery.get("text_destinations") or 0) != 0:
        problems.append(f"text-mode production destinations remain: {delivery.get('text_destinations')}")
    if args.expected_destinations is not None and int(delivery.get("photo_destinations") or 0) != int(args.expected_destinations):
        problems.append(f"expected {args.expected_destinations} photo destinations; found {delivery.get('photo_destinations')}")
    ...
    with db.connect() as con:
        unresolved = con.execute(
            "SELECT COUNT(*) FROM queue WHERE campaign_id=? AND status IN ('pending','retry','deferred','sending','uncertain')",
            (args.campaign_id,),
        ).fetchone()[0]
        global_unresolved_rows = con.execute(
            "SELECT id,campaign_id,status,group_id,content_id FROM queue WHERE status IN ('pending','retry','deferred','sending','uncertain') ORDER BY id"
        ).fetchall()
...
def build_parser():
    ...
    a=sp.add_parser("go-live-readiness"); a.add_argument("campaign_id",nargs="?",default="main_production_01"); a.add_argument("--collection",default="all_approved"); a.add_argument("--canary-campaign",default="album_canary_01"); a.add_argument("--visual-receipt",default="runtime/canary/album_canary_visual_ok.json"); a.add_argument("--expected-destinations",type=int); a.add_argument("--expected-variants",type=int); a.add_argument("--require-album-items",type=int); a.add_argument("--expected-interval-minutes",type=int); a.add_argument("--require-admin-bot",action="store_true"); a.set_defaults(func=cmd_go_live_readiness)
