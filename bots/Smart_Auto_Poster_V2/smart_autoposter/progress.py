...
_STAGE_META = {
    "pending": {"percent": 10, "stage": "QUEUED", "icon": "ðŸ•’", "terminal": False},
    "deferred": {"percent": 25, "stage": "DEFERRED", "icon": "â¸", "terminal": False},
    "retry": {"percent": 35, "stage": "RETRY", "icon": "ðŸ”", "terminal": False},
    "processing": {"percent": 45, "stage": "PREPARING", "icon": "âš™ï¸", "terminal": False},
    "sending": {"percent": 65, "stage": "SENDING", "icon": "ðŸ“¤", "terminal": False},
    "uncertain": {"percent": 90, "stage": "VERIFY", "icon": "âš ï¸", "terminal": True},
    "sent": {"percent": 100, "stage": "SENT", "icon": "âœ…", "terminal": True},
    "failed": {"percent": 100, "stage": "FAILED", "icon": "âŒ", "terminal": True},
    "quarantined": {"percent": 100, "stage": "QUARANTINED", "icon": "ðŸ›‘", "terminal": True},
    "cancelled": {"percent": 100, "stage": "CANCELLED", "icon": "ðŸš«", "terminal": True},
    "expired": {"percent": 100, "stage": "EXPIRED", "icon": "âŒ›", "terminal": True},
}
...
def status_stage(status: str | None) -> dict:
    ...
    meta = _STAGE_META.get(key, {"percent": 0, "stage": key.upper() or "UNKNOWN", "icon": "â€¢", "terminal": False})
...
def render_progress_text(
    ...
    *,
    ...
    lines = [
        ("ðŸ“Š AUTO-POST PROGRESS" if emoji else "AUTO-POST PROGRESS"),
        f"Campaign: {snapshot.get('campaign_id')}",
        f"Run: {_short(snapshot.get('run_key'), 72)}",
        f"Overall {progress_bar(pct, bar_width)} {pct}%",
        (
    ...
    for job in snapshot.get("jobs", []):
        ...
        arrow = "â†’" if emoji else "->"
        job_bar_width = 8 if compact else 10
        stage_width = 16 if compact else 22
        line = (
            f"{prefix}#{job.get('id')} {progress_bar(job.get('stage_percent',0), job_bar_width)} "
