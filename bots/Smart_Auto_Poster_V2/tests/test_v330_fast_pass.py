...
class V330FastPassTests(unittest.TestCase):
    ...
    def test_album_timeout_scales_for_slow_ten_photo_upload(self):
        worker = Worker(None, None, send_timeout_seconds=45)
        self.assertEqual(worker.effective_send_timeout(0), 60)
        self.assertEqual(worker.effective_send_timeout(10), 420)
        overridden = Worker(None, None, send_timeout_seconds=600)
        self.assertEqual(overridden.effective_send_timeout(10), 600)
