...
class V352ProgressTests(unittest.TestCase):
    ...
    def test_green_progress_bar_matches_percentage(self):
        self.assertEqual(progress_bar(0, 10, green=True), "⬜" * 10)
        self.assertEqual(progress_bar(50, 10, green=True), ("🟩" * 5) + ("⬜" * 5))
        self.assertEqual(progress_bar(100, 10, green=True), "🟩" * 10)
    ...
    def test_admin_progress_stays_within_telegram_message_limit_for_32_posts(self):
        ...
        self.assertIn("📊 AUTO-POST PROGRESS · LIVE", text)
        self.assertIn("🟩🟩🟩🟩⬜", text)
