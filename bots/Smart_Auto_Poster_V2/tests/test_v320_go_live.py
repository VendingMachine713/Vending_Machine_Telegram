...
class V320GoLiveTests(unittest.TestCase):
    ...
    def test_v341_autostart_is_battery_safe_and_runtime_probe_is_network_aware(self):
        root = Path(__file__).parents[1]
        autostart = (root / "INSTALL_AUTOSTART.ps1").read_text(encoding="utf-8-sig")
        self.assertIn("-AllowStartIfOnBatteries", autostart)
        self.assertIn("-DontStopIfGoingOnBatteries", autostart)
        self.assertIn("-MultipleInstances IgnoreNew", autostart)

        verify = (root / "VERIFY_RUNTIME.ps1").read_text(encoding="utf-8-sig")
        self.assertIn("watchdog --require service --require scheduler --require worker --json-only", verify)
        self.assertIn("watchdog --require admin_bot --json-only", verify)
        self.assertIn("[DEGRADED_NETWORK]", verify)
        self.assertIn("$network.status -eq 'error'", verify)
        self.assertIn("queue is preserved", verify)
