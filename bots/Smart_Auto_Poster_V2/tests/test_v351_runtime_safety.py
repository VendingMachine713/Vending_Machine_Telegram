...
class V351WindowsLivenessTests(unittest.TestCase):
    ...
    def test_canary_resume_reuses_managed_runtime_instead_of_competing_for_session(self):
        root = Path(__file__).resolve().parents[1]
        text = (root / "RESUME_ALBUM_CANARY.ps1").read_text(encoding="utf-8-sig")
        self.assertIn("runtime\\telegram_runtime.lock\\owner.json", text)
        self.assertIn("Get-ManagedRuntimeOwner", text)
        self.assertIn("Wait-ManagedCanary", text)
        self.assertIn("No competing worker will be started", text)
        self.assertIn("if ($managedOwner)", text)
        self.assertIn("else {\n        & py .\\app.py worker --once", text)
        self.assertIn("finally {", text)
        self.assertIn("campaign-state album_canary_01 paused", text)
