...
class ExpiredCache:
    ...
    async def get(self, files):
        return ["expired-reference"] * len(files)
...
class V305AlbumSendTests(unittest.IsolatedAsyncioTestCase):
    async def test_stalled_media_cache_falls_back_to_local_album_before_destination_send(self):
        pool = TelegramPool(1, "hash", {"primary": "unused"}, media_cache_timeout_seconds=0.01)
        fake = FakeClient()
        pool.clients["primary"] = fake
        pool.media_caches["primary"] = HangingCache()
        files = [f"photo_{i}.jpg" for i in range(10)]
        stages = []

        ids = await pool.send(
            "primary", -1001, "caption", files, "photo",
            stage_callback=lambda phase, percent, detail=None: stages.append((phase, detail)),
        )

        self.assertEqual(ids, list(range(1, 11)))
        self.assertEqual(len(fake.calls), 1)
        self.assertEqual(fake.calls[0][1], files)
        self.assertTrue(any(phase == "media_cache_timeout" for phase, _ in stages))
    ...
    async def test_expired_cached_reference_is_invalidated_and_uploaded_once(self):
        class ExpiringClient(FakeClient):
            async def send_file(self, entity, files, caption=None, **kwargs):
                self.calls.append((entity, files, caption, kwargs))
                if files and files[0] == "expired-reference":
                    raise RuntimeError("RPCError 400: FILE_REFERENCE_0_EXPIRED")
                return [FakeMessage(i + 1) for i in range(len(files))]
        ...
        self.assertEqual(client.calls[1][1], files)
