...
## Per-post pipeline
Typical phases are: queued → validating destination → timing check → content validation → account selection → payload preparation → Telegram send boundary → destination resolution → media preparation/upload (photo) or text send → acknowledgement → sent.
