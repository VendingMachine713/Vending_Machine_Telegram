...
def fmt_row(r):
    who = "@"+r["sender_username"] if r["sender_username"] else (r["display_name"] or str(r["sender_id"] or "?"))
...
async def search_cmd(update, context, cross=False, force_ads=False):
    ...
    if force_ads: raw=(raw+" --ads --available").strip()
    q=parse_query(raw)
...
async def cmd_search(update,context): await search_cmd(update,context,False,False)
async def cmd_cross(update,context):
    if not is_admin(update):
        await update.effective_message.reply_text("Admin only.")
        return
    await search_cmd(update,context,True,False)
async def cmd_ads(update,context): await search_cmd(update,context,True,True)
async def health(update,context):
    await update.effective_message.reply_text(f"✅ Universal Search v1.0\nIndexed: {store.count()} messages")
async def index_message(update,context):
    m=update.effective_message
    if not m or not update.effective_chat: return
    u=update.effective_user
    text=m.text or m.caption or ""
    if not text and not m.effective_attachment: return
    ...
    store.upsert(
        ...
        m.message_id, dt, text, bool(m.effective_attachment)
    )
