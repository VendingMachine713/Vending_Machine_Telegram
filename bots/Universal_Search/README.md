...
v1.6 adds explicit owner feedback (`/matchfeedback`) and aggregate calibration
telemetry (`/matchengine`). Feedback is durable and deduplicated per owner;
thresholds never change automatically.
...
/matchfeedback DEMAND_CHAT DEMAND_MESSAGE SUPPLY_CHAT SUPPLY_MESSAGE positive|negative
/matchengine
```
