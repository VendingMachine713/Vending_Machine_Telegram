...
class RecoveryHistory:
    """Small durable state store for restart-loop prevention.

    It stores only VM recovery metadata, never bot credentials or delivery data.
    Writes use atomic replacement so an interrupted recovery pass does not leave a
    partially written JSON file.
    """

    def __init__(self, root: Path, *, base_cooldown_seconds: int = 120, max_attempts: int = 3, window_seconds: int = 3600):
        self.root = Path(root)
    ...
    def record_attempt(self, service: str, *, action: str, success: bool | None = None, now: datetime | None = None) -> dict[str, Any]:
        now = now or _utcnow()
        data = self.load()
