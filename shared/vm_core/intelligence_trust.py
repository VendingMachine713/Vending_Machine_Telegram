...
def source_trust(source: str) -> float:
    ...
    Namespaced VM Core producers inherit the explicit ``vm_core`` trust entry so
    canonical subcomponents do not accidentally fall back to unknown-source trust.
    Unknown sources deliberately retain a conservative non-zero default.
    """
    name = source.strip()
    if not name:
...
def verify_evidence_provenance(
    ...
    *,
    ...
    Verification never initializes, migrates or creates the platform database.
    Missing stores/tables fail closed with explicit reasons.
    """
