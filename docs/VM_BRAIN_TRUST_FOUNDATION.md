...
`verify_evidence_provenance()` reads the shared event ledger using SQLite `mode=ro` plus `PRAGMA query_only=ON`.

It does not call `PlatformDB.init()`, create a missing database, migrate schema or write platform state. Missing or unreadable stores fail closed with explicit reasons:
...
The shared trust module owns the common canonical ID shape:
...
Downstream intelligence layers should use these shared helpers at Brain boundaries rather than adding bot-local identity validators or leaking raw Telegram/contact IDs.
...
Mission Control v4 exposes this under `trust_foundation`, with headline canonical-subject coverage and an attention count for non-canonical intelligence subjects. This is diagnostic only; legacy/non-canonical events are not rewritten automatically.

## Platform Foundation v4 compatibility

Trust health is additive to the Platform Foundation v4 Mission Control envelope. Service registry, health contracts, incident/intelligence aggregation and contract version 4 remain authoritative and unchanged.

## Safety invariants

The trust foundation keeps these disabled:
...
Later intelligence layers should consume these shared trust and canonical-ID helpers rather than creating isolated bot-level trust logic.
